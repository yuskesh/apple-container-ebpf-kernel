...
wget https://cdn.kernel.org/pub/linux/kernel/v7.x/linux-7.1.12.tar.xz
tar -xf linux-7.1.12.tar.xz
```
...
cp arch/arm64/boot/Image /work/output/Image-7.1.12-ebpf
```
...
```sh
container system kernel set --binary ./output/Image-7.1.12-ebpf --arch arm64 --force
...
```sh
container run --rm docker.io/library/alpine:3.24 uname -r          # -> 7.1.12-ebpf
